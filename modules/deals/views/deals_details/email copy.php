...
<!-- <select multiple="multiple" required="" placeholder="<?= lang('email_to') ?>" name="email_to[]" style="width: 100%" class="select_2_to">
    <option value=""></option>
    <?php
    $all_user_attendees = $this->db->get('tbl_users')->result();

    foreach ($all_user_attendees as $key => $user) {
        if (!empty($user->activated) && $user->activated == 1) {
            $users = $this->mailbox_model->check_by(array('user_id' => $user->user_id), 'tbl_account_details');
            if (!empty($user)) {
                if ($user->role_id == 1) {
                    $role = lang('admin');
    ...
    ?>
                <option value="<?php echo $user->email ?>"><?php echo $users->fullname . ' (<small>' . $role . '</small> )' ?></option>
    <?php
            }
        }
    }
    ...
    ?>
        <option value="<?= $deals_email_details->email_to ?>" selected=""><?= $deals_email_details->email_to ?></option>
    <?php
    }
    ?>
