        ...
        if ($sales_info) {
            foreach ($sales_info as $key => $v_item) :
                ...
                <tr class="sortable item" data-item-id="" id="<?= $v_item->items_id ?>">
                    ...
                    <td>
                        <?= anchor('admin/deals/manuallyItems/' . $id . '/' . $v_item->items_id, '<i class="fa fa-edit"></i> ', 'data-toggle="modal" class="btn btn-primary btn-xs" data-target="#myModal_lg"'); ?>
                        
                        <?= btn_delete('admin/deals/delete_items/' . $v_item->items_id . '/' . $id, null, true) ?>
                        <!--                        --><?php //echo ajax_anchor(base_url("admin/deals/delete_items/" . $v_item->items_id), "<i class='btn btn-xs btn-danger fa fa-trash-o'></i>", array("class" => "itemDel", "title" => lang('delete'), "data-fade-out-on-success" => "#" . $v_item->items_id));
                        ?>
        ...
        <div class="col-sm-4 pv">
            <div class="clearfix">
                ...
                <p class="pull-right mr">
                    <?= $sub_total ? display_money($sub_total) : '0.00' ?>
                </p>
            </div>
            ...
            <div class="clearfix">
                ...
                <p class="pull-right mr">
                    <?= display_money($total, default_currency()) ?>
                </p>
            </div>
        </div>
    </div>
