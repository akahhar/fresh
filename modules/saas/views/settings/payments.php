            ...
            <div class="panel-body">
                ...
                <?php if (!empty($payment)) : ?>
                    ...
                    <form role="form" id="form"
                          action="<?php echo base_url(); ?>saas/settings/save_payments/<?= $payment ?>" method="post"
                          class="form-horizontal  ">
                        <?php $payment_info = get_row('tbl_online_payment', array('gateway_name' => $payment));

                        for ($key = 1; $key <= 5; $key++) {
                            $field = 'field_' . $key;
                            echo get_form_field($payment_info->$field, 'saas');
                        }
                        $status = $this->config->item('saas_' . slug_it(strtolower($payment)) . '_status');
                        ?>
                        <div class="form-group">
                            <label class="col-lg-3 control-label"><?= lang('status') ?></label>
                            <div class="col-lg-6">
                                <select name="<?= 'saas_' . slug_it(strtolower($payment)) . '_status' ?>"
                                        class="form-control">
                        ...
                        <table class="table table-striped DataTables " id="Transation_DataTables">
                            ...
                            foreach ($payment_method as $v_payments) { ?>
                                <tr>
                                    ...
                                    <td><?php
                                        $status = $this->config->item('saas_' . slug_it(strtolower($v_payments->gateway_name)) . '_status');
                                        if ($status == 'active') {
                                            ?>
                                    ...
                                    <td><a data-toggle="tooltip" title="<?= lang('edit') ?>"
                                           ...
                                           href="<?= base_url() ?>saas/settings/payments/<?= $v_payments->gateway_name ?>"><i
