...
class Media extends Gb_Controller
{
    ...
    public function index()
    {
        ...
        $data['dropzone'] = true;
        $data['subview'] = $this->load->view('frontcms/media/index', $data, TRUE);
    ...
    {
        $data['title'] = lang('new') . ' ' . lang('media'); //Page title
        $created = super_admin_access('media', 'created');
        if (!empty($created)) {
            $data['dropzone'] = true;
    ...
    public function save_media()
    {
        ...
        if (!empty($created)) {
            ...
            $target_path = module_dirPath(SaaS_MODULE) . "uploads/";
