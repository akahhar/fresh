(function () {
    ...
    $('.add_share_button').on('click', function () {
        ...
        if (id != '') {
            ...
            requestGet(admin_url + 'spreadsheet/get_my_folder/' + id).done(function (response) {
                    ...
                    if (response.client_groups_share || response.clients_share) {
                        ...
                        $.each(clients_share, function (index, value) {
                            ...
                            if (index > 0) {
                                ...
                                requestGet(admin_url + 'spreadsheet/get_my_folder_get_hash/client/' + value + '/' + id).done(function (response_client) {
                    ...
                    if (response.departments_share || response.staffs_share) {
                        ...
                        $.each(staffs_share, function (index, value) {
                            ...
                            if (index > 0) {
                                ...
                                requestGet(admin_url + 'spreadsheet/get_my_folder_get_hash/staff/' + value + '/' + id).done(function (response_staff) {
    ...
    $('#ShareModal [type="submit"]').on('click', function () {
        var id = $('#ShareModal [name="id"]').val();
        if (id == '') {
            toastr['warning']('Please select a shared file or folder!');
        }
    })

    $('#RelatedModal [type="submit"]').on('click', function () {
        var id = $('#RelatedModal [name="id"]').val();
        if (id == '') {
            toastr['warning']('Please select a file or folder!');
        }
    })
    ...
    $(document).on("dblclick", "#spreadsheet-advanced .tr-pointer", function () {
        ...
        if (type == "file") {
            if (share == false) {
                ...
                requestGet(admin_url + 'spreadsheet/get_hash_related/' + related_id + '/' + pathname + '/' + id_set).done(function (response) {
                    response = JSON.parse(response);
                    window.location.replace(admin_url + 'spreadsheet/file_view_share_related/' + response.hash);
                })
            }
        } else {
    ...
    $('.add_file_button').on('click', function () {
        ...
        window.location.replace(admin_url + 'spreadsheet/new_file_view/' + parent_id);
    })
    ...
    var _rel_id = $('select[name="rel_id[0]'), _rel_type = $('select[name="rel_type[0]'),
        _rel_id_wrapper = $('#rel_id_wrapper');
    $('select[name="rel_type[0]"]').on('change', function () {
        var name = $(this).val();
        requestGet(admin_url + 'spreadsheet/get_related/' + _rel_type.val()).done(function (response) {
            response = JSON.parse(response);
    ...
    $(document).ready(function () {
        $('.share-status').on('click', function () {
            ...
            requestGet(admin_url + 'spreadsheet/get_share_staff_client/' + id_set).done(function (response) {
                response = JSON.parse(response);
        ...
        $("body").on('click', '.new_box_information_review_related', function () {
            ...
            newattachment.find('button[role="button"]').remove(); /* added by nx team */
        ...
        $("body").on('click', '.new_box_information_review', function () {
            var addMoreBoxInformationReviewInputKey = $('.list_information_fields_review select[name*="staffs_share"]').length;
            if ($(this).hasClass('disabled')) {
                return false;
            }
        ...
        $("body").on('click', '.new_box_information_review_client', function () {
            var addMoreBoxInformationReviewInputKeyClient = $('.list_information_fields_review_client select[name*="clients_share"]').length;
            if ($(this).hasClass('disabled')) {
                return false;
            }
    ...
    $('.related-to-hanlde').on('click', function () {
            ...
            requestGet(admin_url + 'spreadsheet/get_related_id/' + val).done(function (response) {
                ...
                $.each(response.type, function (index, value) {
                    if (index > 0) {
                        ...
                        requestGet(admin_url + 'spreadsheet/get_related/' + value).done(function (response) {
                            response = JSON.parse(response);
    ...
    $(document).ready(function () {
        $('input[name="group_share_staff"]').on('click', function () {
        ...
        $('input[name="group_share_client"]').on('click', function () {
            if ($(this).prop("checked") == true) {
                $('.choosee-customer').removeClass('hide');
            } else if ($(this).prop("checked") == false) {
    ...
    $('.setting-sent-notifications').on('click', function () {
    ...
    function new_folder() {
        if ($('#name').val() != '') {
            $.post(admin_url + 'projects/folder/' + project_id, {
                name: $('#name').val(), parent: $('#folder_file_id').val()
            }).done(function () {
    ...
    function onchane_handle_department(index, staff_share) {
        $('select[name="departments_share[' + index + ']"]').on('change', function () {
            ...
            if (val != '') {
                requestGet(admin_url + 'spreadsheet/append_value_department/' + val).done(function (response) {
                    response = JSON.parse(response);
                ...
                requestGet(admin_url + 'spreadsheet/get_staff_all').done(function (response_staff) {
                    response_staff = JSON.parse(response_staff);
    ...
    function onchane_handle_client(index, client_share) {
        $('select[name="client_groups_share[' + index + ']"]').on('change', function () {
            ...
            var name = $(this).attr("name");
            if (val != '') {
                requestGet(admin_url + 'spreadsheet/append_value_group/' + val).done(function (response) {
                    response = JSON.parse(response);
                ...
                requestGet(admin_url + 'spreadsheet/get_client_all').done(function (response_client) {
                    response_client = JSON.parse(response_client);
