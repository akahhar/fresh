function exportExcel(luckysheet, title) { // The parameters are luckysheet.getluckysheetfile() Obtained object
    ...
    return workbook.xlsx.writeBuffer().then(function (buffer) {
        saveAs_s(new Blob([buffer], {type: "application/octet-stream"}), title + '.xlsx');
    });
}
...
function saveDownloadExcel(luckysheet, title, disposition) { // The parameters are luckysheet.getluckysheetfile() Obtained object
    ...
    return workbook.xlsx.writeBuffer().then(function (buffer) {
        ...
        $.ajax({
            url: admin_url + 'spreadsheet/test', type: 'POST', dataType: 'json', data: formData, cache: false
...
var fillConvert = function (bg) {
    ...
    var fill = {
        type: 'pattern', pattern: 'solid', fgColor: {argb: bg.replace('#', '')}
    }
    return fill
}
...
var alignmentConvert = function (vt = 'default', ht = 'default', tb = 'default', tr = 'default') { // luckysheet:vt(垂直), ht(水平), tb(换行), tr(旋转)
    const luckyToExcel = {
        vertical: {
            0: 'middle', 1: 'top', 2: 'bottom', default: 'top'
        }, horizontal: {
            0: 'center', 1: 'left', 2: 'right', default: 'left'
        }, wrapText: {
            0: false, 1: false, 2: true, default: false
        }, textRotation: {
            0: 0, 1: 45, 2: -45, 3: 'vertical', 4: 90, 5: -90, default: 0
        }
    }
