...
// General helper function for $.get ajax requests
    function requestGet(uri, params) {
        ...
        var options = {
            type: 'GET', url: uri.indexOf(admin_url) > -1 ? uri : admin_url + uri
        };
...
// Your Menu Class Name
    ...
    var contextMenuClassName = "context-menu", contextMenuItemClassName = "context-menu__item",
        contextMenuLinkClassName = "context-menu__link", contextMenuActive = "context-menu--active";
    ...
    var menuState = 0, menuWidth, menuHeight, menuPosition, menuPositionX, menuPositionY, windowWidth, k = 0,
        windowHeight;
    ...
    function toggleMenuOn() {
        if (menuState !== 1) {
            ...
            if (type == "file") {
                ...
                if (share == true) {
                    if (role == 1) {
                        ...
                        length_edit == 0 ? (
                                ...
                                $('.context-menu__items').append('<li class="context-menu__item"><a href="#" class="context-menu__link" data-action="edit"><i class="fa fa-cog"></i> ' + edit + ' </a></li>'), $('.context-menu__items').append('<li class="context-menu__item"><a href="#" class="context-menu__link" data-action="d_file"><i class="fa fa-download"></i> ' + download_file + '</a></li>'))

                            : ($('.context-menu__link[data-action="edit"]').parent().remove(), $('.context-menu__link[data-action="d_file"]').parent().remove(), $('.context-menu__items').append('<li class="context-menu__item"><a href="#" class="context-menu__link" data-action="edit"><i class="fa fa-cog"></i> ' + edit + ' </a></li>'), $('.context-menu__items').append('<li class="context-menu__item"><a href="#" class="context-menu__link" data-action="d_file"><i class="fa fa-download"></i> ' + download_file + '</a></li>')
                ...
                if (share == false) {
                    length == 0 ? ($('.context-menu__items').append('<li class="context-menu__item"><a href="#" class="context-menu__link" data-action="create_file"><i class="fa fa-plus"></i> ' + create_file + '</a>'), $('.context-menu__items').append('<li class="context-menu__item"><a href="#" class="context-menu__link" data-action="create_folder"><i class="fa fa-plus"></i> ' + create_folder + '</a>')) : ($('.context-menu__link[data-action="create_file"]').remove(), $('.context-menu__link[data-action="create_folder"]').remove(), $('.context-menu__link[data-action="d_file"]').remove(), $('.context-menu__link[data-action="d_zip"]').remove(), $('.context-menu__items').append('<li class="context-menu__item"><a href="#" class="context-menu__link" data-action="create_file"><i class="fa fa-plus"></i> ' + create_file + '</a>'), $('.context-menu__items').append('<li class="context-menu__item"><a href="#" class="context-menu__link" data-action="create_folder"><i class="fa fa-plus"></i> ' + create_folder + '</a>'))
                } else {
                    if (role == 1) {
                        $('.context-menu__link[data-action="edit"]').parent().remove();
                    } else {
                        var length_edit = $('.context-menu__link[data-action="edit"]').length;
                        length_edit == 0 ? ($('.context-menu__items').append('<li class="context-menu__item"><a href="#" class="context-menu__link" data-action="edit"><i class="fa fa-cog"></i> ' + edit + ' </a></li>')) : ($('.context-menu__link[data-action="edit"]').parent().remove(), $('.context-menu__items').append('<li class="context-menu__item"><a href="#" class="context-menu__link" data-action="edit"><i class="fa fa-cog"></i> ' + edit + ' </a></li>'))
                    }
                }
            }
        ...
        switch (moveToAlbumSelectedId) {
            case 'edit':
                ...
                if (type == 'file') {
                    ...
                    if (share == false) {
                        window.location.replace(admin_url + 'spreadsheet/new_file_view/' + parent_id + '/' + id_set);
                    } else {
                        requestGet(admin_url + 'spreadsheet/get_hash_staff/' + id_set).done(function (response) {
                            response = JSON.parse(response);
                            window.location.replace(admin_url + 'spreadsheet/file_view_share/' + response.hash);
                        })
                    }
                } else {
                    ...
                    if (share == true) {
                        ...
                        length_submot_share == 0 ? $('#AddFolderModal .modal-footer').append(`<button type="submit" class="btn btn-info">Submit</button>`) : ($('#AddFolderModal .modal-footer [type="submit"]').remove(), $('#AddFolderModal .modal-footer').append(`<button type="submit" class="btn btn-info">Submit</button>`));
                    }
                    $('#AddFolderModal').modal('show');
                }
                break;
            case 'delete':
                $.post(admin_url + 'spreadsheet/delete_folder_file/' + id_set).done(function (response) {
                    response = JSON.parse(response);
                    if (response.success == true) {
                        window.location.replace(admin_url + 'spreadsheet/manage');
                    } else {
                        window.location.replace(admin_url + 'spreadsheet/manage');
                    }
                });
            ...
            case 'share':
                ...
                if (id_set != '') {
                    ...
                    requestGet(admin_url + 'spreadsheet/get_my_folder/' + id_set).done(function (response) {
                        response = JSON.parse(response);
                        $('.share-row').removeClass('hide');
                        if (response.clients_share) {
                            ...
                            if (response.client_groups_share || response.clients_share) {
                                ...
                                $.each(clients_share, function (index, value) {
                                    ...
                                    if (index > 0) {
                                        ...
                                        requestGet(admin_url + 'spreadsheet/get_my_folder_get_hash/client/' + value + '/' + id_set).done(function (response_client) {
                            ...
                            if (response.departments_share || response.staffs_share) {
                                ...
                                $.each(staffs_share, function (index, value) {
                                    ...
                                    if (index > 0) {
                                        ...
                                        requestGet(admin_url + 'spreadsheet/get_my_folder_get_hash/staff/' + value + '/' + id_set).done(function (response_staff) {
            ...
            case "related":
                ...
                if (val != '') {
                    requestGet(admin_url + 'spreadsheet/get_related_id/' + val).done(function (response) {
                        response = JSON.parse(response);
                        $.each(response.type, function (index, value) {
                            if (index > 0) {
                                ...
                                requestGet(admin_url + 'spreadsheet/get_related/' + value + '/' + response.id[index]).done(function (response) {
            ...
            case "d_zip":
                ...
                requestGet(admin_url + 'spreadsheet/get_folder_zip/' + id_set + '/' + nameCleaned).done(function (response) {
                    response = JSON.parse(response);
                })
                break;
            case "d_file":
                requestGet(admin_url + 'spreadsheet/get_file_sheet/' + id_set).done(function (response) {
                    response = JSON.parse(response);
