<script type="text/javascript">
    ...
    (function () {
        ...
        function milestone_kanban_init() {
            ...
            $all_task_info = get_order_by('tbl_task', array('project_id' => $project_details->project_id), 'milestones_order', true);
            ...
            foreach ($m_milestones as $m_v_milestone) {
                $tt = $this->db->where('milestones_id', $m_v_milestone->milestones_id)->order_by('milestones_order', 'ASC')->get('tbl_task')->result();
            ...
            $total_comments = total_rows('tbl_task_comment', array('module' => 'tasks', 'module_field_id' => $mv_task->task_id));
            $total_attachment = total_rows('tbl_attachments', array('module' => 'tasks', 'module_field_id' => $mv_task->task_id));
            $text = null;
            $due_date = $mv_task->due_date;
            $due_time = strtotime($due_date);
            $current_time = strtotime(date('Y-m-d'));
            $text .= '<a data-milestones-id="' . $mid . '"   data-task-id="' . $mv_task->task_id . '" style="word-wrap: break-word;" class="text-info" href="' . base_url() . 'admin/tasks/details/' . $mv_task->task_id . '">' . clear_textarea_breaks($mv_task->task_name) . '</a>';
