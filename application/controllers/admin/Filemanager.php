...
//        if ($this->input->is_ajax_request()) {
        ...
        $isSaaS = apply_filters('is_saas', 0);
        ...
        $root_options = array(
            ...
            'path' => set_realpath($mainPath),
            'URL' => site_url($mainPath) . '/',
            ...
            'disabled' => (empty($isSaaS) ? [] : ['archive', 'chmod', 'duplicate', 'extract', 'file', 'get', 'mkdir', 'mkfile', 'paste', 'rename', 'resize', 'rm', 'upload', 'zipdl']),
            'uploadOrder' => array(
                'allow',
                'deny'
            ),
        ...
        if ($this->session->userdata('user_type') == 3) {
            ...
            $path = set_realpath($mainPath . $user->media_path_slug);
            ...
            $root_options['URL'] = site_url($mainPath . '/' . $user->media_path_slug) . '/';
        }
        $opts = array(
            'roots' => array(
                $root_options
